...
function buddyboss_theme_child_scripts_styles()
{
  ...
  wp_enqueue_style('dashboard-parent-css', get_stylesheet_directory_uri() . '/assets/css/dashboard-parent.css');
...
if (!function_exists('ir_custom_hide_products_tile')) {
  /** * Hide products tile on instructor dashboard * * @author Kumar Rajpurohit <kumar.rajpurohit@wisdmlabs.com> */ function ir_custom_hide_products_tile()
  {
    if (!function_exists('wdm_is_instructor') || !wdm_is_instructor()) {
      return;
    } ?> <style>
      .ir-addon-info-tile {
        display: none;
            ...
            add_action('admin_head', 'ir_custom_hide_products_tile');
          }
        ...
        */
          ...
          /*function get_id_child_parent($user_id)
          {
            global $wpdb;
            ...
            $results = $wpdb->get_results("SELECT child_id FROM `wp_ldap_child_parent` where parent_id=" . $user_id); 
          ...
          */
          ...
          //Unlink course
          function ldpa_unlink_child_course($post, $parent_id)
          {
            if (!wp_verify_nonce($_POST['_wpnonce'], 'ld-unlink-course')) {
              echo 'Did not save because your form seemed to be invalid. Sorry';
              return;
            }

            if (empty($_POST['course_id']) || empty($_POST['parent_id']) || empty($_POST['child_id'])) {
              exit('Error');
            }

            if (isset($_POST['course_id']) || isset($_POST['parent_id']) || isset($_POST['child_id'])) {
              $child_id = (int) $_POST['child_id'];
              $user = get_user_by('id', $child_id);
              if ($user) {
              } else {
                echo'Invalid email address, Please double check email.';
              }
            }
          }
          //Obtenir les cours du parent

          function get_parent_vault_courses($parent_id)
          {

            global $wpdb;
            $table_name_course = $wpdb->prefix . "ldap_course_storage";
            $query = $wpdb->get_results(
              "SELECT id, parent_id, course_id, order_id
        ...
        WHERE parent_id = {$parent_id}"
          ...
          // function ldpa_submit_link_post($post, $parent_id)
          // {
          //   if (!wp_verify_nonce($_POST['_wpnonce'], 'ld-submit-links')) {
          //     echo 'Did not save because your form seemed to be invalid. Sorry';
          //     return;
          //   }

          //   if (empty($_POST['link_child_email'])) {
          //     exit('Error');
          //   }

          //   if (isset($_POST['link_child_email'])) {
          //     $email = sanitize_email($_POST['link_child_email']);
          //     $user = get_user_by('email', $email);
          //     if ($user) {
          //       $studentIds = array($user->ID);
          //       global $wpdb;
          //       $table = $wpdb->prefix . 'ldap_child_parent';
          //       $exist = $wpdb->get_results("SELECT child_id FROM {$table} WHERE child_id = {$user->ID} LIMIT 1", ARRAY_N);
          //       if ($exist) {
          //         echo'This email is already linked';
          //       } else {
          //         $data = array(
          //           'parent_id' => $parent_id,
          //           'child_id' => $user->ID
          ...
          //         echo'Account successfully linked.';
          //         unset($_POST);
          //       }
          //     } else {
          //       echo'Invalid email address, Please double check email.';
...
/* 
          Delete linked child
          function ldpa_delete_link_post($post, $parent_id)
          {
         
            
            if (!wp_verify_nonce($_POST['_wpnonce'], 'ld-remove-links')) {
              echo 'Did not save because your form seemed to be invalid. Sorry';
              return;
            }

            if (empty($_POST['linked_child_id'])) {
              exit('Error');
            }

            if (isset($_POST['linked_child_id'])) {
              global $wpdb;
              $table = $wpdb->prefix . 'ldap_child_parent';
              $child_id = (int) sanitize_title($_POST['linked_child_id']);
              $child_detail = get_user_by('id', $child_id);
              $wpdb->query(
                $wpdb->prepare(
                  "
                    DELETE FROM {$table}
              ...
              //$this->message( $child_detail->user_email . ' successfully removed.', 'green');
              unset($_POST);
